from configs import ArcfaceConfig, BaseConfig, CenterlossConfig, SpherefaceConfig, RFWDataConfig, BFWDataConfig, VggFace2Config, VMERDataConfig
from data import Balancedface, BalancedfaceSingle, BalancedfacePair, BalancedfaceMulti, RFW, getRFWList, BFWTrain, BFWTest, getBFWList, VMERTrain, VMERTest, getVMERList
...
dataset_list_str = ', '.join(list(dataset_lookup_dict.keys()))
...
def get_trainset(opt):
    ...
    # Race distribution variation
    elif 'race_simplex' in opt['experiment_name']:
        with open(opt['simplex_file_path'], 'r') as json_file:
            ...
            return get_race_simplex_trainset(opt)
    raise ValueError(f'Unsupported experiment: {opt["experiment_name"]}')


def get_race_simplex_trainset(opt):
    TrainDataset = dataset_lookup_dict[opt['dataset']]['train']
    if opt['dataset'] == 'BFW':
        return TrainDataset(opt['train_root'], opt['identities_per_race'], pair_list_file=os.path.join(opt['bfw_test_list']), seed=opt['trial_num'])
    elif opt['dataset'] == 'RFW':
        face_ratios_file = None
        if 'filtered_face_ratios_list' in opt:
            face_ratios_file = opt['filtered_face_ratios_list']            
        return TrainDataset(opt['train_root'], opt['identities_per_race'], face_ratios_file=face_ratios_file, seed=opt['trial_num'])
    elif opt['dataset'] == 'VMER':
        return TrainDataset(opt['train_root'], opt['identities_per_race'], balanced_train_json=opt['balanced_train_json'], seed=opt['trial_num'])
    

def get_testset(opt):
    TestDataset = dataset_lookup_dict[opt['dataset']]['test']
    if opt['dataset'] == 'BFW':
        return TestDataset(opt['test_root'], opt['test_race'], input_shape=opt['input_shape'], pair_list_file=os.path.join(opt['bfw_test_list']))
    elif opt['dataset'] == 'RFW':
        return TestDataset(opt['test_root'], opt['test_race'], input_shape=opt['input_shape'])
    elif opt['dataset'] == 'VMER':
        return TestDataset(opt['test_root'], opt['test_race'], input_shape=opt['input_shape'], balanced_test_json=opt['balanced_test_json'])


def get_test_pair_list(opt):
    if opt['dataset'] == 'BFW':
        return getBFWList(os.path.join(opt['bfw_test_list']), opt['test_race'],)
    elif opt['dataset'] == 'RFW':
        return getRFWList(os.path.join(opt['test_root'], 'txts', opt['test_race'], opt['test_race'] + opt['rfw_test_list']))
    elif opt['dataset'] == 'VMER':
        return getVMERList(os.path.join(opt['vmer_test_list_root'], opt['test_race'] + opt['vmer_test_list']))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='RFW', type=str, help=f'Dataset; one of: {dataset_list_str}')
    ...
    if opt['test']:
        ...
        test_loader = data.DataLoader(test_dataset,
                                      ...
                                      shuffle=False,
                                      num_workers=opt['num_workers'])
        test_pair_list = get_test_pair_list(opt)
