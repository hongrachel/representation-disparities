...
class Trainer(object):
    ...
    def load_model(self):
        ...
        elif 'race_simplex' in self.opt['experiment_name']:
    ...
    def validate(self, data_loader):
        ...
        with torch.no_grad():
            for ii, batch in enumerate(data_loader):
                ...
                accs.append(acc)
                if ii % self.opt['print_freq'] == 0 or ii == len(data_loader) - 1:
    ...
    def calc_accuracy(self, pairwise_similarities, pair_labels, threshold):
        pair_labels_np = np.asarray(pair_labels)
        predictions = (np.asarray(pairwise_similarities) >= threshold)
        accuracy = np.mean((predictions == pair_labels_np).astype(int))

        # FNR = prediction 0 label 1 / all label 1
        fn_num = np.sum((predictions == False) & (pair_labels_np == True))
        true_pos_num = np.sum(pair_labels_np == True)
        fnr = 1.0 * fn_num / max(1, true_pos_num)

        # FPR = prediction 1 label 0 / all label 0
        fp_num = np.sum((predictions == True) & (pair_labels_np == False))
        true_neg_num = np.sum(pair_labels_np == False)
        fpr = 1.0 * fp_num / max(1, true_neg_num)
    ...
    def compute_accuracy_from_folds(self, pairwise_similarities, pair_labels):
        ...
        for i in range(10):
            ...
            accuracy, fnr, fpr = self.calc_accuracy(val_pairwise_similarities, val_pair_labels, threshold)
            accuracies.append(accuracy)
            fnrs.append(fnr)
            fprs.append(fpr)

        return [np.mean(accuracies), np.mean(fnrs), np.mean(fprs)]
    ...
    def evaluate(self):
        ...
        # accuracy, fnr, fpr = self.compute_accuracy_from_folds(pairwise_similarities, pair_labels)
        print('Threshold, Test Accuracy, Test FNR, Test FPR')
        for i in range(5, 100, 5):
            t = i / 100
            accuracy, fnr, fpr = self.calc_accuracy(pairwise_similarities, pair_labels, t)
            print(t, accuracy, fnr, fpr)
